...
import { Poppins } from "next/font/google"; // Balik ke Poppins
...
export default function DashboardPage() {
  ...
  // --- DATA DUMMY CHART (STATIS) ---
  // Menggunakan data tetap untuk menghindari Error Hydration (Math.random)
  const chartMockData = [
    { month: 'Jul', attendance: 72, resign: 12 },
    { month: 'Agu', attendance: 78, resign: 8 },
    { month: 'Sep', attendance: 65, resign: 15 },
    { month: 'Okt', attendance: 85, resign: 5 },
    { month: 'Nov', attendance: 60, resign: 22 },
    { month: 'Des', attendance: 75, resign: 10 },
  ];

  // --- LOGIC: Fetch Data Real-time ---
  const loadData = async () => {
    ...
    try {
      ...
      if (crewData && predData) {
        // Hitung Stats
        const total = crewData.filter(c => c.is_active).length;
        const high = predData.filter((p: any) => p.risk_score > 75).length;
        ...
        setOutletRisks(heatmapArray.slice(0, 3)); // Ambil Top 3 Outlet
        setHighRiskList(predData.slice(0, 5)); // Ambil Top 5 Crew
      }
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const runPrediction = async () => {
    ...
    try {
        await fetch('/api/analytics/predict', { method: 'POST' });
        toast.success("Analisis Selesai! Data diperbarui.");
        loadData();
    } catch (error) {
  ...
  return (
    // Menggunakan warna background Beige (#f4e3be) & Font Poppins
    <div className={`min-h-screen bg-[#f4e3be] text-[#022020] pb-10 ${poppins.className}`}>
    
        {/* --- MAIN CONTENT (Padding top disesuaikan karena header fixed) --- */}
        <main className="w-full max-w-[1440px] mx-auto p-6 md:p-8 pt-32 flex flex-col gap-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <header className="fixed top-0 left-0 right-0 z-50 px-6 md:px-8 py-4">
            ...
            <section className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {outletRisks.length === 0 && <div className="col-span-3 text-center py-10 text-gray-400 italic">Belum ada data prediksi. Silakan klik "Run AI Prediction".</div>}

                    {outletRisks.map((outlet, idx) => {
                        const style = getOutletStyle(outlet.avgRisk);
                        // URL Gambar Dummy Kota
                        const bgImage = [
            ...
            <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-8">
                ...
                {/* LEFT: CHART (Fixed Hydration Error) */}
                <div className="bg-white rounded-[1.5rem] p-8 shadow-sm border border-white/50 flex flex-col">
                    ...
                    <div className="flex-1 min-h-[300px] relative flex items-end justify-between gap-4 px-2 pb-6 border-b border-l border-[#022020]/10">
                        ...
                        {/* Data Bars Mock - MENGGUNAKAN DATA STATIS AGAR TIDAK ERROR HYDRATION */}
