...
type PayrollDraft = {
  crew_id: string;
  full_name: string;
  outlet_id: string; // Tambahkan ini
  outlet_name: string;
  bank_name: string;
  ...
  const handleGenerateDraft = async () => {
    ...
    try {
      const { data: crews } = await supabase
        .from("crew")
        .select(`
          id, full_name, outlet_id, bank_name, bank_account_number,
          outlets(name),
      ...
      const draft: PayrollDraft[] = crews.map((crew) => {
        ...
        crewLoans.forEach(loan => {
            ...
            const rencana_potong = Number(loan.deduction_plan_amount);
            total_remaining_debt_start += sisa;
            let actual_cut = rencana_potong;
            if (sisa < rencana_potong) actual_cut = sisa;
            total_planned_deduction += actual_cut;
        });
        ...
        const uang_makan = ((abs?.count_h || 0) + count_ht) * Number(contract.daily_meal_allowance);

        return {
          crew_id: crew.id,
          full_name: crew.full_name,
          outlet_id: crew.outlet_id, // Simpan ID Outlet
          // @ts-ignore
          ...
          net_salary: (gapok + uang_makan + rankBonus) - (total_deduction_sia + total_planned_deduction)
        };
      });

      setPayrollData(draft);
      setIsCalculated(true);
      toast.success("Data ditarik!");
    ...
    try {
      const payload = payrollData.map(d => {
        ...
        return {
            ...
            crew_id: d.crew_id,
            outlet_id: d.outlet_id, // Simpan ID Outlet ke Database
            ...
            notes: [d.rank ? `Juara ${d.rank}` : null, d.has_sick_letter ? "Lampiran Surat Sakit" : null].filter(Boolean).join(", "),
            status: 'finalized'
        };
      });
                            ...
                            <TableRow key={row.crew_id}>
                                <TableCell>
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                       ...
                                       {row.rank && <div className="flex items-center text-yellow-600"><Trophy className="h-3 w-3 fill-yellow-500"/><span className="text-[9px] font-bold">#{row.rank}</span></div>}
                                    </div>
                                </TableCell>
                                <TableCell>
                                    <div className="truncate w-16">{row.bank_name}</div>
                                ...
                                <TableCell>
                                    ...
                                    {row.rank_bonus_amount > 0 && <div className="text-[9px] text-green-600 mt-0.5">Termasuk Insentif: {formatRp(row.rank_bonus_amount)}</div>}
                                </TableCell>
                                ...
                                <TableCell><Input type="number" className="h-7 text-xs px-1 text-red-600 font-bold bg-red-50" value={row.deduction_sia} onChange={e => updateField(idx, 'deduction_sia', Number(e.target.value))}/></TableCell>
                                ...
                                <TableCell><Input type="number" className="h-7 text-xs px-1 bg-gray-100" value={row.remaining_loan} readOnly /></TableCell>
                                <TableCell className="text-right font-bold text-blue-700 text-sm bg-blue-50">{formatRp(row.net_salary)}</TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
