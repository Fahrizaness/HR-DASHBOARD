...
export default function DashboardPage() {
  ...
  // DATA DUMMY CHART (Simulasi tren 6 bulan terakhir)
  const chartMockData = [
  ...
  const handleInvestigate = (outletName: string) => {
      ...
      setTimeout(() => {
          ...
          toast.success(`Filter aktif: ${outletName}`);
      }, 1000);
  };
        ...
        <main className="w-full max-w-[1440px] mx-auto px-6 md:px-8 flex flex-col gap-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-[1.5rem] shadow-sm border border-white/50 flex flex-col justify-between h-44 group hover:-translate-y-1 hover:shadow-xl transition-all duration-300">
                    ...
                    <div className="flex items-center gap-2 mt-auto"><span className="bg-[#078832]/10 text-[#078832] text-xs font-bold px-2 py-1 rounded-lg flex items-center gap-1"><TrendingUp className="w-3 h-3" /> Data Realtime</span></div>
                </div>
                <div className="bg-white p-6 rounded-[1.5rem] shadow-sm border border-white/50 flex flex-col justify-between h-44 group hover:-translate-y-1 hover:shadow-xl transition-all duration-300">
                    <div className="flex justify-between items-start">
                        <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-2 mt-auto"><span className="text-[#022020]/50 text-sm font-medium">Rata-rata probabilitas resign</span></div>
                </div>
                <div className="bg-white p-6 rounded-[1.5rem] shadow-sm border border-[#d6302a]/30 flex flex-col justify-between h-44 group hover:-translate-y-1 hover:shadow-xl transition-all duration-300 relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-2 mt-auto relative z-10">
                        <span className={`bg-[#d6302a]/10 text-[#d6302a] text-xs font-bold px-2 py-1 rounded-lg flex items-center gap-1 ${stats.highRisk === 0 ? 'bg-green-100 text-green-700' : ''}`}>{stats.highRisk > 0 ? `+${stats.highRisk} orang` : 'Aman'}</span>
                        <span className="text-[#022020]/50 text-sm font-medium">{stats.highRisk > 0 ? "Perlu atensi segera" : "Kondisi stabil"}</span>
                    </div>
                </div>
            </div>
            ...
            <section className="space-y-6">
                <div className="flex items-center justify-between">
                    ...
                    <Link href="/admin/outlets" className="text-sm font-bold text-[#033f3f] hover:underline flex items-center gap-1 cursor-pointer bg-white/50 px-4 py-2 rounded-lg border border-white/50 hover:bg-white transition-colors">Lihat Data Outlet <ArrowRight className="w-4 h-4" /></Link>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {loading ? [1,2,3].map(i => <div key={i} className="h-[320px] bg-white/50 animate-pulse rounded-[1.5rem]"></div>) : outletRisks.length === 0 ? (
                        <div className="col-span-3 text-center py-12 text-gray-400 italic bg-white/50 rounded-xl border border-dashed border-gray-300">Belum ada data prediksi. Silakan klik tombol "Run AI Prediction" di pojok kanan atas.</div>
                    ) : (
                            ...
                            return (
                                <div key={idx} className={`group bg-white rounded-[1.5rem] overflow-hidden border-2 ${style.border} shadow-md hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 flex flex-col h-[320px]`}>
                                    ...
                                    <div className="p-6 flex flex-col gap-4 flex-1">
                                        ...
                                        <button onClick={() => handleInvestigate(outlet.name)} className={`mt-auto w-full py-3 flex items-center justify-center gap-2 bg-opacity-10 ${style.bg.replace('bg-', 'bg-opacity-10 ')} ${style.color} hover:${style.bg} hover:text-white font-bold text-sm rounded-xl transition-all active:scale-95`}>Investigasi <ChevronRight className="w-4 h-4"/></button>
                                    </div>
                                </div>
                            )
                        })
